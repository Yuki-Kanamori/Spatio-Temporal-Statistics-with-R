...
needs::needs(leaps, lmtest, nlme, ape, broom, FRK, tidyverse, lattice, RColorBrewer, gstat, sp, spacetime, STRbook,
             pforeach, magrittr)
...
G <- Tmax %>% 
  select(lon, lat) %>% 
  SpatialPoints() %>% 
  auto_basis(data = ., nres = 1, type = "Gaussian")
G
...
Tmax_no_14 %<>% mutate(residuals = residuals(Tmax_July_lm))
...
g <- ggplot(filter(Tmax_no_14, day %in% 24:31)) +
  ...
  col_scale(name = "degF") +
  geom_point(data = filter(Tmax_no_14, day %in% 24:31 & id %in% c(3810, 3889)),
             aes(lon, lat), colour = "black", pch = 2, size = 2.5) +
  theme_bw()
...
days <- c(1:13, 15:31)                      # set of days
P <- pforeach(i = seq_along(days), .combine = rbind)({                 # for each day
  Tmax_day <- filter(Tmax_no_14,
                     day == days[i])        # filter by day
  station.dists <- Tmax_day %>%             # take the data
    select(lon, lat) %>%                    # extract coords.
  ...
  station.dists.inv <- 1/station.dists      # weight matrix
  diag(station.dists.inv) <- 0              # 0 on diag
  Moran.I(Tmax_day$residuals,               # run Moran's I
          station.dists.inv) %>%
  do.call("cbind", .)                       # conv. to df
}) %>% data.frame()
...
Tmax_no_14 %>% filter(id %in% c(3810, 3889)) %>% 
  ggplot(aes(x = day, y = residuals, colour = as.factor(id))) +
  geom_point() + geom_line() +
  labs(x = "day of July 1993", y = "residuals (degF)", colour = "spatial location") +
  theme_bw()
...
## ------------------------------------------------------------------------
purrr::map(nested_Tmax_no_14$data, dwtest_one_station) %>% head()
...
pred_grid <- predict(Tmax_July_lm,
                       newdata = pred_grid,
                       interval = "prediction") %>% 
  cbind(pred_grid) %>% 
  mutate(z_err = (upr - lwr) / (2 * 1.96)) %>% 
  rename(z_pred = fit)
